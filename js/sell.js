/* ===========================================================
   Bike.X — sell.js (COMPLETE REFACTORED)
   All data flows through DataService.
   Images are stored as raw File objects (not base64) 
   for future Supabase Storage compatibility.
   =========================================================== */

// ---------- GLOBALS ----------
let pendingImageFiles = [];   // Raw File objects (for upload later)
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  if (typeof BRANDS !== 'undefined' && typeof CITIES !== 'undefined') {
    ...
    CITIES.forEach(function(c) {
      const option = document.createElement('option');
      option.value = c;
      option.textContent = cityLabel(c);
...
function setupSellCategorySwitcher() {
  ...
  const requiredFields = {
    motorcycles: ["brand"],
    parts: ["partTitle", "partType"],
    gear: ["gearTitle", "gearType"],
  ...
  function applySellCategoryUI() {
    ...
    if (conditionRow) conditionRow.style.display = (sellCategory === "services") ? "none" : "";
...
/* ---------- IMAGE UPLOAD (stores raw File objects) ---------- */
...
function renderImagePreviews() {
  ...
  document.querySelectorAll('.remove-image-btn').forEach(function(btn) {
    btn.addEventListener('click', function(e) {
      ...
      renderImagePreviews();
      // Revoke object URLs to free memory (done automatically on page unload)
    });
  });
}
...
function handleSellSubmit(e) {
  ...
  if (sellCategory === "motorcycles") {
    ...
    // We are NOT converting images to base64. We store the raw File objects.
    // For now, we keep them as File references; later DataService can upload them.
    newListing = {
      id: "U" + Date.now(),
      category: "motorcycles",
      brand: brand, model: model, year: year, cc: cc,
      price: price, mileage: mileage, city: city || null, color: color,
      condition: condition, commentsEnabled: commentsEnabled, desc: enteredDesc, descAr: enteredDesc,
      variant: Math.floor(Math.random() * 3),
      images: pendingImageFiles.map(f => URL.createObjectURL(f)), // Temporary URLs for preview
    ...
    newListing = {
      ...
      price: price, city: city || null, condition: condition, commentsEnabled: commentsEnabled,
      desc: enteredDesc, descAr: enteredDesc,
      variant: Math.floor(Math.random() * 3),
      images: pendingImageFiles.map(f => URL.createObjectURL(f)),
      createdAt: new Date().toISOString(),
      creatorEmail: user ? user.email : null,
      creatorName: user ? user.name : null
    };
    ...
    newListing = {
      id: "U" + Date.now(),
      category: "gear",
      title: gearTitle, subtitle: gearSize ? `${gearType} · Size ${gearSize}` : gearType,
      price: price, city: city || null, condition: condition, commentsEnabled: commentsEnabled,
      desc: enteredDesc, descAr: enteredDesc,
      variant: Math.floor(Math.random() * 3),
      images: pendingImageFiles.map(f => URL.createObjectURL(f)),
    ...
    newListing = {
      ...
      price: price, city: city || null, commentsEnabled: commentsEnabled,
      desc: enteredDesc, descAr: enteredDesc,
      variant: Math.floor(Math.random() * 3),
      images: pendingImageFiles.map(f => URL.createObjectURL(f)),
      createdAt: new Date().toISOString(),
