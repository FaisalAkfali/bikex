...
/* ---------- Rendering ---------- */
let state = {
  search:"", brand:"", city:"", condition:"", minPrice:"", maxPrice:"",
...
function renderGrid(){
  ...
  filtered.forEach(l => {
    ...
    if (category !== "motorcycles") {
      ...
      html += `
      <article class="card" data-id="${l.id}" tabindex="0" role="button" aria-label="${l.title}">
        <div class="card-media" style="background:linear-gradient(160deg, ${color}1a, transparent);">
          ${categoryIconTile(category)}
          ${l.condition ? `<span class="badge ${l.condition === 'used' ? 'used' : ''}">${l.condition === 'new' ? t('conditionNew') : t('conditionUsed')}</span>` : ''}
        ...
        <div class="card-body">
          ...
          <div class="card-meta">
            <span>${cityDisplay}</span>${l.subtitle ? `<span>&middot;</span><span>${l.subtitle}</span>` : ''}
          </div>
        </div>
      </article>`;
      return;
    ...
    <article class="card" data-id="${l.id}" tabindex="0" role="button" aria-label="${titleText} listing">
      <div class="card-media" style="${mainImage ? 'background:var(--surface-2);' : 'background:linear-gradient(160deg, ' + color + '1a, transparent);'}">
        ${mediaContent}
        <span class="badge ${l.condition === 'used' ? 'used' : ''}">${l.condition === 'new' ? t('conditionNew') : t('conditionUsed')}</span>
...
function wireEvents(){
  ...
  if (filterCity) {
    filterCity.addEventListener("change", function(e) { 
  ...
  if (clearFilters) {
    clearFilters.addEventListener("click", function() {
      state = { search:"", brand:"", city:"", condition:"", minPrice:"", maxPrice:"", sort:"newest", onlyFavorites:state.onlyFavorites, category:state.category };
...
function updateMobileAuth() {
  ...
  if (navAuthEl) {
    if (user) {
      navAuthEl.innerHTML = `
        <a href="dashboard.html" class="mobile-nav-link" id="mobileNavDashboard">📊 ${t("dashboardBtn")}</a>
        <a href="messages.html" class="mobile-nav-link" id="mobileNavMessages">💬 ${t("messages")}</a>
        <button type="button" class="mobile-nav-link mobile-nav-btn" id="mobileNavLogout">🚪 ${t("logoutBtn")}</button>
      `;
      ...
      navAuthEl.innerHTML = `<a href="login.html" class="mobile-nav-link" id="mobileNavLogin">🔑 ${t("loginLink")}</a>`;
    }
  }
